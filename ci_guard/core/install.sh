...
function ccb_download_binarys() {
    ...
    for ((i = 1; i <= MAX_RETRIES; i++)); do
        echo "Attempt $i/$MAX_RETRIES: Downloading $package from $project (arch: $arch)"
        
        if [[ -d $binary_folder ]]; then
            echo "Cleaning up previous download folder: $binary_folder"
            sudo rm -rf ./$binary_folder
        fi
        ccb download os_project=$project packages=$package architecture=$arch -d -b all
        exit_code=$?
    ...
    if [[ $download_success -ne 1 ]]; then
        echo "Failed to download the RPM package generated by $package project $project arch $arch after $MAX_RETRIES attempts."
        exit 1
    fi
