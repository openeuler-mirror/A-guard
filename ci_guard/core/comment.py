...
class Comment:
    ...
    def __init__(self, pr_url, message, process, users):
        ...
        self.atomgit_api = Atomgit(repo=self.repo_name)
    ...
    def _add_comment_to_pr(self, message, choice=False):
        ...
        response = self.atomgit_api.create_pr_comment(number=self.pr_num, body=message)
        if not response:
    ...
    def _modify_comment_to_pr(self, message, process_comment_id):
        ...
        """
        response = self.atomgit_api.modify_pr_comment(
