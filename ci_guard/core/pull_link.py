...
class Pull:
    ...
    def _pull_comment(pr, repo, body):
        atomgit_api = Atomgit(repo=repo)
        atomgit_api.create_pr_comment(number=pr, body=body)
    ...
    def _pre_check_link(self, target_repo, target_pr, pr_number, source_repo):
        pr_info = Atomgit(repo=target_repo).get_single_pr_info(number=target_pr)
        if not pr_info:
            logger.warning(f"Target pr info not found: {target_pr}.")
            self._pull_comment(
                pr_number,
                source_repo,
                f"> 目标PR不存在:https://atomgit.com/src-openeuler/{target_repo}/pull/{target_pr}",
            )
        ...
        if pr_info.get("state") == "merged":
            ...
            self._pull_comment(
                pr_number,
                source_repo,
                f"> 目标PR已合入:https://atomgit.com/src-openeuler/{target_repo}/pull/{target_pr}",
            )
        ...
        if pr_info.get("state") == "close":
            ...
            self._pull_comment(
                pr_number,
                source_repo,
                f"> 目标PR关闭:https://atomgit.com/src-openeuler/{target_repo}/pull/{target_pr}",
            )
            return dict(link_result="failed", detail="目标PR关闭")
        url = f"https://atomgit.com/src-openeuler/{source_repo}/"
        ...
        if not build_depend or target_repo.lower() not in relation_packages:
            ...
            self._pull_comment(
                pr_number,
                source_repo,
                f"> 待关联的PR不存在依赖:https://atomgit.com/src-openeuler/{target_repo}/pull/{target_pr}",
            )
    ...
    def pull_link(self, pr_number, source_repo, target_pr, target_repo):
        ...
        if source_repo in list(breadth_tree_nodes["tree_nodes"]):
            ...
            msg = f"源PR已存在关联关系:https://atomgit.com/src-openeuler/{_repo}/pull/{_pr} 请联系对应仓maintainer确认"
            logger.warning(msg)
        ...
        try:
            ...
            self._pull_comment(
                pr_number,
                source_repo,
                f"> 目标Pr:https://atomgit.com/src-openeuler/{target_repo}/pull/{target_pr} 已关联",
            )
            ...
            self._pull_comment(
                pr_number,
                source_repo,
                f"> 目标PR关联错误:https://atomgit.com/src-openeuler/{target_repo}/pull/{target_pr}",
            )
            return dict(link_result="failed", detail="目标PR关联错误")

        for repo, pull in {source_repo: pr_number, target_repo: target_pr}.items():
            Atomgit(repo=repo).create_tag(pr_number=pull, body=["linkpull"])

        return dict(
            link_result="success",
            detail=f"目标Pr:https://atomgit.com/src-openeuler/{target_repo}/pull/{target_pr} 已关联",
        )
    ...
    def relation_verify(self, pr_number, repo):
        ...
        for pr_link_info in link_prs:
            if (
                ...
                link = "be_link_pr"
            pr_info = Atomgit(repo=repo).get_single_pr_info(number=number)
    ...
    def _can_merge(self, repo, pr_number):
        ...
        for repo, link_status in breadth_tree_nodes["link_merged"].items():
            pull_link = (
                f"https://atomgit.com/src-openeuler/{repo}/pull/{link_status['pr']}"
            )
    ...
    def _merge_pull(self, pull):
        ...
        if not Atomgit(repo=repo).remove_tag(_pr, "linkpull"):
            self._pull_comment(
                _pr,
                repo,
    ...
    def synchronous_merge(self, pr_number, repo):
        ...
        Link pull merged
        """
        tags = Atomgit(repo=repo).get_all_tag(
    ...
    def forced_merge(self, pr_number, repo):
        ...
        merged_pulls["meet_merge"].add(
            f"https://atomgit.com/src-openeuler/{repo}/pull/{pr_number}"
        )
        # delete link tag
    ...
    def download_kernel_repo_of_tag(self, pr_number, src_openeuler_ulr, repo):
        ...
        command(
            cmds=[
                ...
                "--depth",
                "1",
                f"https://atomgit.com/openeuler/kernel",
