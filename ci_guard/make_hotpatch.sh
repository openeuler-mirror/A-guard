...
function download_hotpatch_rpm(){
    ...
    do
        ...
        wget -r -l1 -nd -A 'patch*.xml' $url/hotpatch_xml/ -P ${hotpatch_path}/${arch}/hotpatch_xml/
        wget -r -l1 -nd -A 'patch*.rpm' $url/Packages/ -P ${hotpatch_path}/${arch}/Packages/
        if [[ $arch == "x86_64" ]]; then
            mkdir -p ${hotpatch_path}/source/Packages ${hotpatch_path}/source/hotpatch_xml
            wget -r -l1 -nd -A '*.src.xml' $url/hotpatch_xml/ -P ${hotpatch_path}/source/hotpatch_xml/
            wget -r -l1 -nd -A '*.src.rpm' $url/Packages/ -P ${hotpatch_path}/source/Packages/
        fi
    done
}
